...
soft3dEngine::soft3dEngine(HWND hwnd, uint16 width, uint16 height)
{
	m_hWnd = hwnd;
	m_width = width;
	m_height = height;
...
int soft3dEngine::DrawPixel(uint16 x, uint16 y, uint32 value, uint16 size)
{
	...
	for (uint16 i = 0; i < size; i++)
	{
		if (i % 2 == 1)
		{
			for (uint16 j = 0; j < size; j++)
				SetOperateBuffer((x+j) * m_width + y + (i+1)/2, value);
		}
		else
		{
			for (uint16 j = 0; j < size; j++)
				SetOperateBuffer((x+j) * m_width + y - i/2, value);
		}
	}
	return 0;
}

void soft3dEngine::SetOperateBuffer(uint32 index, uint32 value)
{
